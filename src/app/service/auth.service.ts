...
export class AuthService {
  ...
  user: Observable<any>;
  public url='http://localhost:4000/api/users';
  ...
  CreateUser(userInterface:userInterface):any{
    const url_api = "";
    return this.http.post(this.url+'/register',userInterface)
      //{headers: this.headers})
      //.pipe(map(data => data));
  }

  loginUser( email: string, password: string): any{
    const api_consult = this.url;
    return this.http.post(api_consult, {email,password})
    .pipe(map(data => data));
  }
